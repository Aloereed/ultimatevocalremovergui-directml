...
dml=torch_directml.device() if torch_directml.is_available() else torch.device('cpu')
...
class MainWindow(TkinterDnD.Tk if is_dnd_compatible else tk.Tk):
    ...
    def __init__(self):
        ...
        self.is_gpu_available = torch_directml.is_available() if not OPERATING_SYSTEM == 'Darwin' else torch.backends.mps.is_available()
    ...
    def determine_auto_chunks(self, chunks, gpu):
        ...
        if chunks == 'Full':
            ...
            if gpu == 0:
                gpu_mem = round(torch_directml.gpu_memory()[0]/1.074e+9)
                if gpu_mem <= int(6):
                    chunk_set = int(5)
